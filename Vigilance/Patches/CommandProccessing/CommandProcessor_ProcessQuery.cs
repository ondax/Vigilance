	...
	public static class CommandProcessor_ProcessQuery
	{
		public static bool Prefix(string q, CommandSender sender)
		{
			try
			{
				...
				{
					sender.RaReply($"SERVER#You are not allowed to use __instance command ({(ConfigManager.EnableGameCommands ? "Blacklisted command or UserID" : "Command disabled in config")})!", false, true, "");
					return false;
				}

				if (ConfigManager.IsBlacklisted(sender, query))
				{
					sender.RaReply($"SERVER#You are not allowed to use __instance command!", false, true, "");
					return false;
				}
			...
			catch (Exception e)
            {
				...
				sender.RaReply($"SERVER#An error occured while processing __instance command!\nError: {e.Message}", false, true, "");
				return false;
            }
		}
	}
