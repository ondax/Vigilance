		...
		public static bool Prefix(PlayerMovementSync __instance, out bool wasChanged)
		{
			try
			{
				...
				if (__instance._ensnared.Enabled || Math.Abs(num) < 1E-05f)
				{
					if (magnitude > 1f)
					{
						__instance.ForcePosition(__instance._lastSafePosition, "S.4", false, true);
						wasChanged = true;
					}
				...
				if (__instance._hub.characterClassManager.CurClass == RoleType.Scp173 && !__instance._hub.characterClassManager.Scp173.AllowMove)
				{
					...
					if (__instance._scp173AllowTime <= 0f)
					{
						...
						{
							__instance.ForcePosition(__instance._lastSafePosition, "S.5", false, true);
							wasChanged = true;
							return false;
						}
				...
				if (__instance._receivedPosition.y > 1500f)
				{
					...
					{
						__instance.ForcePosition(__instance._lastSafePosition, "S.3", false, true);
						wasChanged = true;
						return false;
					}
				...
				else
				{
					...
					else
					{
						...
						if (!__instance.Grounded || (__instance._receivedPosition.y < __instance.RealModelPosition.y && Math.Abs(__instance._receivedPosition.y - __instance.RealModelPosition.y) < 0.4f))
						{
						...
						{
							__instance.ForcePosition(__instance._lastSafePosition, "S.1", false, false);
							wasChanged = true;
							return false;
						}
					...
					IL_3F6:
						...
						{
							__instance.ForcePosition(__instance._lastSafePosition, "S.2", false, true);
							wasChanged = true;
							return false;
						}
						if (num2 > num3 && Vector3.Distance(__instance.RealModelPosition, __instance._lastSafePosition) > num4)
						{
							__instance.ForcePosition(__instance._lastSafePosition, "S.6", false, true);
							wasChanged = true;
							return false;
						}
						...
						if (__instance._hub.characterClassManager.CurClass != RoleType.Scp106 && !__instance._hub.scp106PlayerScript.goingViaThePortal)
						{
							...
							{
								__instance.ForcePosition(__instance._lastSafePosition, "R.3", false, false);
								wasChanged = true;
								return false;
							}
						}
